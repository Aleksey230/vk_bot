            ...
            if event.type == VkEventType.MESSAGE_NEW:
                    if event.to_me:
                            if event.text.lower () == 'начать':
                                ...
                                if example2 == []:
                                    vk.method('messages.send', {'user_id':event.user_id,'message': 'Ваш баланс 0 mathcoins', 'random_id':0})
                                else:
                                    vk.method('messages.send', {'user_id':event.user_id,'message':'Ваш баланс ' + str(example2['balance']) + ' mathcoins', 'random_id':0})
                            elif event.text.lower() == 'сдаюсь':
                                example = collection.find({'user_id': event.user_id, 'by_me': True}).sort('time', -1).limit(1)[0]
                                answer = example['answer']
                                vk.method('messages.send', {'user_id':event.user_id,'message': answer, 'random_id':0})
                            elif event.text.lower() == 'новичок':
                                example2 = collection.find({'user_id': event.user_id, 'to_me': True}).sort('time', -1).limit(1)[0]
                                hard = 1
                                gen = novice_gen(hard)
                                answer = gen[1]
                                s = {'time': event.timestamp, 'user_id': event.user_id, 'message': event.text, 'hard' : 1, 'EXP': 0, 'to_me': True, 'user_hard': 'новичок', 'balance': example2['balance'], 'novice': example2['novice'], 'lover': example2['lover'], 'master': example2['master']}
                                ...
                                gen = master_gen(hard)
                                answer = gen[1]                                
                                s = {'time': event.timestamp, 'user_id': event.user_id, 'message': event.text, 'hard' : 1, 'EXP': 0, 'to_me': True, 'user_hard': 'мастер', 'balance': example2['balance'], 'novice': example2['novice'], 'lover': example2['lover'], 'master': example2['master']}
                                collection.insert_one(s)
                                ...
                                gen = lover_gen(hard)
                                answer = gen[1]
                                s = {'time': event.timestamp, 'user_id': event.user_id, 'message': event.text, 'hard' : 1, 'EXP': 0, 'to_me': True, 'user_hard':'любитель', 'balance': example2['balance'], 'novice': example2['novice'], 'lover': example2['lover'], 'master': example2['master']}
                                collection.insert_one(s)
                                a = {'time': event.timestamp, 'user_id': event.user_id, 'message': gen[0], 'answer': str(answer), 'by_me': True}
                                collection.insert_one(a)
                                ...
                                gen = novice_gen(hard)
                                answer = gen[1]
                                s = {'time': event.timestamp, 'user_id': event.user_id, 'message': event.text, 'hard' : hard, 'EXP': checkTrue, 'to_me': True, 'user_hard': user_hard, 'balance': 0, 'novice': 500, 'lover': 1000, 'master':2000}
                                collection.insert_one(s)
                                a = {'time': event.timestamp, 'user_id': event.user_id, 'message': gen[0], 'answer': answer, 'by_me': True,}
                                collection.insert_one(a)
                                vk.method('messages.send', {'user_id':event.user_id,'message': gen[0], 'random_id':0})
                                ...
                                if str(event.text) == str(answer):
                                        if example2['EXP'] >= 9:
                                                ...
                                                s = {'time': event.timestamp, 'user_id': event.user_id, 'message': str(event.text), 'hard' : example2['hard'], 'EXP': example2['EXP'], 'to_me': True, 'user_hard':user_hard, 'balance': balance, 'novice': novice, 'lover': lover, 'master': master}
                                                collection.insert_one(s)
                                        else:
                                                example2['EXP'] += 1
                                                vk.method('messages.send', {'user_id':event.user_id,'message': 'Правильно!', 'random_id':0})
